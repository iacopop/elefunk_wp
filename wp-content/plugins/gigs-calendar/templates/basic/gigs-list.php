<div class="gigs-calendar" style="margin-top:50px;">
	<table class="gigs calendar <?php echo $upcoming ? 'upcoming' : 'archive' ?>">
		<?php if ( $upcoming == 'upcoming' ){
                    echo '<caption>Prossimi concerti</caption>';
                }else{
                    echo '<caption>Concerti passati</caption>';
                }
                ?>
		...
		<tbody>
			<?php foreach ( $gigs as $gkey => $g ) : ?>
				<?php foreach ( $g->performances as $key => $p ) : ?>
					<tr id="performance-<?php echo $p->id; ?>" >
						<?php
							if ( $key == 0 ) {
								$fields = array(
